...
- **Then**:
  - UI: Sara has one unread notification: `Bug assigned to you: Checkout total rounds incorrectly`.
  - Link: clicking the notification opens the bug detail with attached run/test context visible.
  - DB/System: one notification exists for Sara for that assignment event.
...
- **Then**:
  - UI: Mateo receives `Bug assigned to you: Checkout total rounds incorrectly`.
  - UI: Sara receives no removal notification in this Story.
  - DB/System: no notification is created for the previous assignee unless they are also reporter and the event is a status change.
...
- **Then**:
  - UI: Sara sees a notification that the bug status changed to `in progress`.
  - UI: notification text includes the bug title and status transition when BK-31 exposes both values.
  - DB/System: one notification is created for Sara for that status-change event.
...
- **Then**:
  - UI: Sara sees a status-change notification for that assigned bug.
  - Link: clicking it opens the bug detail with retest/run context available.
  - DB/System: one notification is created for Sara for that status-change event.
...
- **Then**:
  - UI: no notification about that change appears in Sara's inbox.
  - DB/System: no notification row is created for Sara for her own action.
...
- **Then**:
  - UI: no notification about that self-change appears in Sara's inbox.
  - DB/System: no notification row is created for Sara for her own action.
...
- **Then**:
  - UI: Mateo receives no notification about it.
  - DB/System: no notification row is created for Mateo for that event.
...
1. **Can BK-212 be developed before BK-31 ships the bug lifecycle?**
   - **Context**: Story depends on assignment/status-change events.
   - **Impact if unanswered**: Dev may build fake event plumbing or untestable assumptions.
   - **PO answer**: No. BK-212 can be estimated now, but implementation starts only after BK-31 provides the bug entity, assignment, status lifecycle, and event payload contract.

2. **What should happen when a bug has no run/test context attached?**
   - **Context**: Scope says the deep link lands on the bug with attached test/run context.
   - **Impact if unanswered**: QA cannot assert fallback behavior.
   - **PO answer**: Link must always land on bug detail. If run/test context is absent, the bug page shows the available bug details and no broken/empty context panel.

3. **Should previously assigned users be notified when a bug is reassigned away from them?**
   - **Context**: Business rule says no removal notification in this Story.
   - **Impact if unanswered**: Audience scope expands and estimate changes.
   - **PO answer**: No. This Story only notifies the new assignee; previous-assignee removal notifications are out of scope.
...
1. **What should the notification row look like?**
   - **Design answer**: Use bug icon; primary text with bug title; secondary text for `Assigned to you` or `Status changed: <old> -> <new>`; severity chip reused from BK-31 once available.

2. **How should status transitions render?**
   - **Design answer**: Compact transition copy `<old> -> <new>` when both values exist. If BK-31 only provides next status, render `Status changed to <new>`.

3. **How should inaccessible or deleted bug links behave visually?**
   - **Design answer**: Do not show inaccessible notifications in the inbox. If a stale already-open row is clicked after access loss, show the standard not-found/permission-safe state, never bug metadata leakage.
