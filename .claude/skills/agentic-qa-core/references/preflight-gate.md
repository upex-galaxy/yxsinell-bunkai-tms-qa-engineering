...
|---|---|---|
| **Framework adapted (artifacts present — NOT a generic boilerplate)** | The OUTPUT that `/adapt-framework` produces is in place. Probe its Phase 9 ADAPTED signals, read-only: `bun run vars:check` exits 0 AND `.agents/project.yaml` has zero `null #` lines; no `Example*` components or `example.types` facade exist; no `module-example` spec dirs remain under `tests/e2e/` / `tests/integration/`. | Still generic → **STOP and hand the user the commands to run THEMSELVES**: `/project-discovery` (if `.context/` is missing) then `/adapt-framework`. The gate **never auto-runs** them — it verifies their artifacts only. |
